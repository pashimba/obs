...
import { _ as Util, a as getNumberArrayValidator, b as glob, c as getNumberValidator, d as DD, f as Canvas, g as Transform, h as Context, i as getBooleanValidator, l as getStringOrGradientValidator, m as SceneCanvas, n as Factory, o as getNumberOrArrayOfNumbersValidator, p as HitCanvas, r as RGBComponent, s as getNumberOrAutoValidator, t as Node, u as getStringValidator, v as Konva$2, y as _registerNode } from "./Node-0ybsHk62.js";
import { t as require_scheduler } from "./scheduler-CSTTkjWa.js";
...
var SUPPORT_POINTER_EVENTS = Konva$2._global["PointerEvent"] !== void 0;
...
var getEventsMap = (eventType) => {
	...
	if (type === "pointer") return Konva$2.pointerEventsEnabled && EVENTS_MAP.pointer;
...
var Stage$1 = class extends Container {
	...
	add(layer, ...rest) {
		...
		layer.draw();
		if (Konva$2.isBrowser) this.content.appendChild(layer.canvas._canvas);
		return this;
	}
	getParent() {
		return null;
	...
	_pointermove(evt) {
		...
		if (Konva$2.isDragging() && DD.node.preventDefault() && evt.cancelable && isTouchPointer) evt.preventDefault();
		this.setPointersPositions(evt);
		if (!(!(Konva$2.isDragging() || Konva$2.isTransforming()) || Konva$2.hitOnDragEnabled)) return;
	...
	_pointerup(evt) {
		...
		this._changedPointerPositions.forEach((pos) => {
			...
			if (shape && shape.isListening()) {
				...
				if (Konva$2["_" + eventType + "ListenClick"]) this._fire(events.pointerclick, {
					evt,
					target: this,
					currentTarget: this,
					pointerId
...
if (Konva$2.isBrowser) document.addEventListener("visibilitychange", () => {
...
Shape$1.prototype.on("shadowColorChange.konva shadowBlurChange.konva shadowOffsetChange.konva shadowOpacityChange.konva shadowEnabledChange.konva", _clearHasShadowCache);
Shape$1.prototype.on("shadowColorChange.konva shadowOpacityChange.konva shadowEnabledChange.konva", _clearGetShadowRGBACache);
Shape$1.prototype.on("fillPriorityChange.konva fillPatternImageChange.konva fillPatternRepeatChange.konva fillPatternScaleXChange.konva fillPatternScaleYChange.konva fillPatternOffsetXChange.konva fillPatternOffsetYChange.konva fillPatternXChange.konva fillPatternYChange.konva fillPatternRotationChange.konva", _clearFillPatternCache);
Shape$1.prototype.on("fillPriorityChange.konva fillLinearGradientColorStopsChange.konva fillLinearGradientStartPointXChange.konva fillLinearGradientStartPointYChange.konva fillLinearGradientEndPointXChange.konva fillLinearGradientEndPointYChange.konva", _clearLinearGradientCache);
Shape$1.prototype.on("fillPriorityChange.konva fillRadialGradientColorStopsChange.konva fillRadialGradientStartPointXChange.konva fillRadialGradientStartPointYChange.konva fillRadialGradientEndPointXChange.konva fillRadialGradientEndPointYChange.konva fillRadialGradientStartRadiusChange.konva fillRadialGradientEndRadiusChange.konva", _clearRadialGradientCache);
...
var Layer$1 = class extends Container {
	...
	_getIntersection(pos) {
		...
		const p3 = p[3];
		if (p3 === 255) {
			const shape = shapes[Util.getHitColorKey(p[0], p[1], p[2])];
			if (shape) return { shape };
...
var Easings = {
	...
	BounceEaseInOut(t, b, c, d) {
		if (t < d / 2) return Easings.BounceEaseIn(t * 2, 0, c, d) * .5 + b;
		else return Easings.BounceEaseOut(t * 2 - d, 0, c, d) * .5 + c * .5 + b;
	},
	EaseIn(t, b, c, d) {
		return c * (t /= d) * t + b;
	},
...
var Path$1 = class Path$1 extends Shape$1 {
	...
	static getPointAtLengthOfDataArray(length, dataArray) {
		...
		const p = cp.points;
		switch (cp.command) {
			case "L": return Path$1.getPointOnLine(length, cp.start.x, cp.start.y, p[0], p[1]);
			case "C": return Path$1.getPointOnCubicBezier(t2length(length, Path$1.getPathLength(dataArray), (i) => {
			...
			case "Q": return Path$1.getPointOnQuadraticBezier(t2length(length, Path$1.getPathLength(dataArray), (i) => {
		...
		for (let n = 1; n < arr.length; n++) {
			...
			for (let j = 0, jlen = coords.length; j < jlen; j++) {
				...
				if (arcParamIndex >= 0) {
					...
					if (!isNaN(parsedArc)) p.push(parsedArc);
					else p.push(0);
					arcParamIndex += 1;
					if (arcParamIndex >= 7) arcParamIndex -= 7;
				} else {
...
var TextPath$1 = class extends Shape$1 {
	...
	_setTextData() {
		...
		for (let i = 0; i < chars.length; i++) {
			...
			const width = Path$1.getLineLength(charStartPoint.x, charStartPoint.y, charEndPoint.x, charEndPoint.y);
			let kern = 0;
			if (kerningFunc) try {
			...
			charEndPoint.x += kern;
			this.textWidth += kern;
			const midpoint = Path$1.getPointOnLine(kern + width / 2, charStartPoint.x, charStartPoint.y, charEndPoint.x, charEndPoint.y);
...
var Transformer$1 = class extends Group$1 {
	...
	_createAnchor(name) {
		...
		anchor.on("mouseenter", () => {
			const cursor = getCursor(name, Konva$2.getAngle(this.rotation()), this.rotateAnchorCursor());
		...
		const back = new Shape$1({
			...
			sceneFunc(ctx, shape) {
				...
				if (tr.rotateEnabled() && tr.rotateLineVisible()) {
					...
					else if (dirY > 0) t = Math.min(t, (height - cy) / dirY);
					if (dirX < 0) t = Math.min(t, -cx / dirX);
					else if (dirX > 0) t = Math.min(t, (width - cx) / dirX);
	...
	_handleMouseMove(e) {
		...
		if (this._movingAnchorName === "rotater") {
			...
			const newRotation = Konva$2.getAngle(this.rotation()) + delta;
...
var Emboss = function(imageData) {
	...
	for (let p = 0, i = 0; i < data.length; i += 4, p++) lum[p] = .2126 * src[i] + .7152 * src[i + 1] + .0722 * src[i + 2];
	const Gx = [
		-1,
		0,
		1,
...
var Solarize = function(imageData) {
	...
	for (let i = 0; i < d.length; i += 4) {
		...
		if (.2126 * r + .7152 * g + .0722 * b >= threshold) {
			d[i] = 255 - r;
			d[i + 1] = 255 - g;
			d[i + 2] = 255 - b;
		}
...
var require_react_reconciler_development = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = function($$$config) {
		...
		function completeWork(current, workInProgress, renderLanes) {
			...
			switch (workInProgress.tag) {
				...
				case 13:
					...
					return null;
				case 4: return popHostContainer(workInProgress), updateHostContainer(current, workInProgress), null === current && preparePortalMount(workInProgress.stateNode.containerInfo), bubbleProperties(workInProgress), null;
		...
		function unwindWork(current, workInProgress) {
			popTreeContext(workInProgress);
			switch (workInProgress.tag) {
				case 1: return current = workInProgress.flags, current & 65536 ? (workInProgress.flags = current & -65537 | 128, (workInProgress.mode & 2) !== NoMode && transferActualDuration(workInProgress), workInProgress) : null;
				...
				case 22:
				case 23: return popSuspenseHandler(workInProgress), popHiddenContext(workInProgress), null !== current && pop(resumedCache, workInProgress), current = workInProgress.flags, current & 65536 ? (workInProgress.flags = current & -65537 | 128, (workInProgress.mode & 2) !== NoMode && transferActualDuration(workInProgress), workInProgress) : null;
		...
		function commitPassiveUnmountEffectsInsideOfDeletedTree_begin(deletedSubtreeRoot, nearestMountedAncestor$jscomp$0) {
			for (; null !== nextEffect;) {
				...
				(current.mode & 2) !== NoMode && 0 <= componentEffectStartTime && 0 <= componentEffectEndTime && (componentEffectSpawnedUpdate || .05 < componentEffectDuration) && logComponentEffect(current, componentEffectStartTime, componentEffectEndTime, componentEffectDuration, componentEffectErrors);
				popComponentEffectStart(prevEffectStart);
				popComponentEffectDuration(prevEffectDuration);
				componentEffectSpawnedUpdate = prevEffectDidSpawnUpdate;
				componentEffectErrors = prevEffectErrors;
...
var HostTransitionContext = /* @__PURE__ */ import_react.createContext(null);
...
* MIT
*/
if ("19.2.4".indexOf("19") === -1) throw new Error("react-konva version 19 is only compatible with React 19. Make sure to have the last version of react-konva and react or downgrade react-konva to version 18.");
